...
| `rooms.js` | **房间层**：房间存储、状态机、快照/广播、建房、**TTL 回收扫描**、**雾幕纠缠度** | `rooms`(Map)、`snapshot/broadcast`、`createRoom/startRound`、`takeAction/nextSlotOf/afterAction`（行动三选一）、`checkFog/entanglementOf`（纠缠度→雾区）、`nextTurn/startPicking/finalizePicking/startReveal`、`room.bg`（背景字段，不经快照）、`levels/acted/slotOwner/actionLog/fogLevels`、`sweepTick/destroyRoom/stopSweep`（回收）、`lastActivity/allOfflineSince` |
...
| `public/game.js` | 阿弥陀籤纯逻辑（前后端共用） | `Game.mapping/resolve/path/pairOf/trackPath` |
...
| **行动三选一**（扳道岔/暗轨/待命）+ 每人 1+1 配额 | `rooms.js startRound`（配额）+ `takeAction`（占槽/扣配额）+ `handlers.js draw_line`（校验 kind） |
| **暗轨隐私**（他人视角空槽、揭晓全显） | `rooms.js snapshot`（按观看者过滤 levels）+ `startReveal`（完整 levels 参与置换） |
| **单轮交错槽归属**（round-robin 预分配 + 一次交接） | `rooms.js startRound`（slotOwner）+ `nextTurn/afterAction/fillPlayerSlots`（交接与托管补齐） |
| 托管（退出自动显手） | `handlers.js leave_room` + `rooms.js nextTurn/autoDraw`（永远 open） |
| 超时（施工自动扳道岔 / 选点补选） | `rooms.js nextTurn`（TURN_MS 超时 autoDraw）+ `startPicking/autoFinalizePicking`（PICK_MS） |
| **夜色雾**（多轮视野窗口，前端纯视觉） | 前端 `board.js`（`FOG_WINDOW`、`drawFog`、揭晓 `revealFogY`）——服务端只提供完整 levels |
| **雾幕纠缠度** | `rooms.js checkFog(room, slot)`（新线落定 → canvas 相邻三行区域 [k-2,k-1,k] 判定；`ENTANGLE/FOG_TRIGGER` 见 config；触发后区域内三行全部进 `fogLevels`，快照过滤 + 公开） |
...
`create_room join_room rejoin update_results set_mode set_round set_maxlines set_fog set_bg start_drawing draw_line{kind:'open'|'dark'|'skip',pair?} end_drawing pick_start leave_room restart reconfigure` ＋ `reveal_finished`（无 ack）＋ 音频通道 `bind / audio`。（`end_turn` 已随 v0.15 单轮改造移除。）
- **校验策略**：全部在 `handlers.js` 服务端权威校验（轮次、阶段、房主、合法性、唯一性、暗轨/待命配额）
- **状态广播**：`rooms.js broadcast()` 按稳定玩家 id 下发 `snapshot(room, forId)`（含私有字段 myPick/myResult；levels 按观看者过滤暗轨）
- **行动反馈**：`takeAction` 后广播快照 + 发 `line_drawn {level, pair, auto, kind}`（客户端据此播放音效）
